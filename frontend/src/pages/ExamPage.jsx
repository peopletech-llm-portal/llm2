...
function ExamPage() {
  ...
  useEffect(() => {
    ...
    if (!sid) {
      setMessage("Please log in to access the exam.");
      navigate("/login");
      return;
    }
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/exams/${id}`, {
      headers: {
  ...
  const handleSubmit = async (auto = false) => {
    ...
    if (!auto && exam?.examType === "coding") {
      const code = answers?.code || exam?.questions?.[0]?.starterCode || "";
      if (!code.trim()) {
    ...
    if (exam?.examType === "coding") {
      payloadAnswers = {
        code: answers?.code || exam?.questions?.[0]?.starterCode || "",
        language: answers?.language || exam?.allowedLanguages?.[0] || "javascript",
      };
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/results/submit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (res.ok) {
        ...
        localStorage.setItem(`exam_attempted_${id}_${studentId}`, "true");
  ...
  const handleRunCode = async () => {
    ...
    const code = answers?.code || exam.questions?.[0]?.starterCode || "";
    const language = answers?.language || exam.allowedLanguages?.[0] || "javascript";
  ...
  const currentLanguage = answers?.language || exam?.allowedLanguages?.[0] || "javascript";
  const starterCode = exam?.questions?.[0]?.starterCode || "function solve(input){\n  return input;\n}";
  const currentCode = answers?.code ?? starterCode;
    ...
    <div className="min-h-screen bg-black text-white p-2 md:p-6 w-full">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-xl shadow-2xl max-w-sm w-full text-center border border-red-500">
            ...
            <p className="text-sm mb-6 text-gray-300">
              {violationReason}. You have {Math.max(0, 1 - violationCount)} warnings left before auto-submission.
            </p>
      ...
      {!isFullscreen && (
        <div className="mb-6 p-4 bg-yellow-900/30 border border-yellow-500 rounded-lg">
          ...
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-700 hover:to-orange-700 text-white rounded-lg font-semibold transition-all duration-300 transform hover:scale-105"
          >
            Enter Fullscreen
          </button>
      ...
      <div className="bg-gray-900 p-4 md:p-6 rounded-xl border border-gray-700 mb-6 w-full max-w-[1920px] mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold mb-4 text-center">
          <span className="bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">{exam.title}</span>
        </h2>
        {timeLeftMs != null && (
          <div className="mb-4 text-center">
            <div className="inline-flex items-center px-3 py-1 md:px-4 md:py-2 bg-red-900/30 border border-red-500 rounded-lg">
              <span className="text-red-400 font-bold text-base md:text-lg">
                ⏰ Time left: {Math.max(0, Math.floor(timeLeftMs / 60000))}m{" "}
                {Math.max(0, Math.floor((timeLeftMs % 60000) / 1000))}s
              </span>
            </div>
          </div>
        )}
      ...
      {exam.examType === "theory" ? (
                ...
                <div className="bg-gray-900 p-4 md:p-6 rounded-xl border border-gray-700 h-full flex flex-col">
                        ...
                        <ul className="list-disc pl-5 text-sm text-gray-300 space-y-2">
                          {exam.questions[0].testcases.slice(0, 5).map((tc, i) => (
                            <li key={i} className="font-mono">
                              <span className="text-blue-400">Input:</span> {String(tc.input)} <span className="text-green-400 ml-2">→ Expected:</span>{" "}
                              {String(tc.expected)}
                            </li>
                          ))}
                        </ul>
                  ...
                  <div className="mt-auto space-y-3">
                    <div className="flex items-center mb-3">
                      ...
                      >
                        {(exam.allowedLanguages?.length ? exam.allowedLanguages : ["javascript", "python", "java", "cpp"]).map((lang) => (
          ...
          {runResult && (
            <div className="mt-6 p-4 border border-gray-600 rounded-xl bg-gray-900 w-full max-w-[1920px] mx-auto">
              <p className="text-sm mb-3">
                ...
                <span className="text-white">{runResult.status}</span>
              </p>
              {runResult.stdout && (
                <div className="mt-3">
                  <p className="text-sm font-semibold text-green-400 mb-2">Output:</p>
                  <pre className="text-xs whitespace-pre-wrap bg-gray-800 p-3 rounded border border-gray-600 text-gray-200 font-mono">
                    {runResult.stdout}
                  </pre>
                </div>
              )}
              {runResult.compile_output && (
                <div className="mt-3">
                  ...
                  <pre className="text-xs whitespace-pre-wrap bg-red-900/30 p-3 rounded border border-red-500 text-red-300 font-mono">
                    {runResult.compile_output}
                  </pre>
                </div>
              )}
            ...
            <div className="mt-6 p-4 border border-gray-600 rounded-xl bg-gray-900 w-full max-w-[1920px] mx-auto">
              ...
              <ul className="text-sm space-y-3">
                {sampleResults.map((r, i) => (
                  <li
                    key={i}
                    className={`p-4 rounded-lg border ${r.passed ? "bg-green-900/30 border-green-500" : "bg-red-900/30 border-red-500"}`}
                  >
                    ...
                    <div className="space-y-1 text-gray-300 font-mono text-xs">
                      <div>
                        <span className="text-blue-400">Input:</span> <code className="bg-gray-800 px-2 py-1 rounded">{r.input}</code>
                      </div>
                      <div>
                        <span className="text-green-400">Expected:</span>{" "}
                        <code className="bg-gray-800 px-2 py-1 rounded">{r.expected}</code>
                      </div>
                      <div>
                        <span className="text-purple-400">Output:</span>{" "}
                        <code className="bg-gray-800 px-2 py-1 rounded">{(r.stdout || "").trim()}</code>
                      </div>
                    </div>
                  </li>
                ))}
        ...
        exam.questions.map((q, qIndex) => (
          <div
            key={qIndex}
            className="mb-8 bg-gray-900 p-6 rounded-xl border border-gray-700 hover:border-gray-600 transition-all duration-300"
          >
            ...
            <div className="space-y-3">
              {q.options.map((opt, optIndex) => (
                <label
                  key={optIndex}
                  className="flex items-center cursor-pointer p-3 rounded-lg border border-gray-600 hover:border-blue-500 hover:bg-gray-800 transition-all duration-300"
                >
                  <input
                    type="radio"
                    name={`q-${qIndex}`}
